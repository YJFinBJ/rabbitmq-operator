...
public class RabbitMQResourceController extends
        AbstractResourceController<RabbitMQCustomResource, RabbitMQCustomResourceList, DoneableRabbitMQCustomResource, Resource<RabbitMQCustomResource, DoneableRabbitMQCustomResource>> {

    public RabbitMQResourceController(final KubernetesClient client, final Map<String, String> labelsToWatch) {
    ...
    protected MixedOperation<RabbitMQCustomResource, RabbitMQCustomResourceList, DoneableRabbitMQCustomResource, Resource<RabbitMQCustomResource, DoneableRabbitMQCustomResource>> operation() {
        final CustomResourceDefinition rabbitCrd = getClient().customResourceDefinitions().withName(RABBITMQ_CRD_NAME)
                .get();

        if (rabbitCrd == null) {
            throw new RuntimeException(
                    String.format("CustomResourceDefinition %s has not been defined", RABBITMQ_CRD_NAME));
        }

        return getClient().customResources(rabbitCrd, RabbitMQCustomResource.class, RabbitMQCustomResourceList.class,
                DoneableRabbitMQCustomResource.class);
    }
}